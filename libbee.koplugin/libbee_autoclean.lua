...
-- ---------------------------------------------------------------------------
-- Document & Reader Inspection
-- ---------------------------------------------------------------------------

--- Checks whether a book file is actively opened in KOReader's ReaderUI.
--- @param file_path string
--- @return boolean
local function isDocumentOpen(file_path)
...
M.isDocumentOpen = isDocumentOpen
...
--- Refuses to remove the file if it is currently open in ReaderUI.
--- @param file_path string
--- @return boolean success, string|nil error_message
function M.removeFileAndSidecar(file_path)
    if not file_path or file_path == "" then return false, "No file path provided" end

    -- Guard: never delete a file that is actively open in ReaderUI (prevents engine crashes)
    if isDocumentOpen(file_path) then
        logger.warn("libbee autoclean: cannot remove file while open in ReaderUI: " .. tostring(file_path))
        return false, "File is currently open in reader"
    end

    -- Remove book file only; preserve .sdr sidecar directory
    pcall(os.remove, file_path)

    -- Invalidate KOReader caches and history safely
    pcall(function()
...
--- @param options table|nil { is_live_sync = boolean, verified_sync = boolean, now_timestamp = number, grace_days = number, synced_account_ids = table }
--- @return table { deleted_count = number, deleted_titles = table, errors = table }
function M.checkAndCleanup(shelf, options)
    options = options or {}
    local shelf_is_table = (type(shelf) == "table")
    local is_live_sync = false
    if options.is_live_sync == true and shelf_is_table and options.verified_sync ~= false then
        -- An empty shelf (#shelf == 0) requires explicit verified_sync=true to prevent accidental wiping
        if #shelf > 0 or options.verified_sync == true then
            is_live_sync = true
        end
    end
    local now_ts = options.now_timestamp or os.time()
    local grace_days = options.grace_days or 1
    local synced_accs = options.synced_account_ids
    ...
    for loan_id, tracked in pairs(registry) do
        ...
        -- If specific accounts were synced, only consider early return for loans belonging to those accounts
        local account_matches = true
        if synced_accs and tracked.account_id and not synced_accs[tracked.account_id] then
            account_matches = false
        end

        if is_live_sync and account_matches then
            ...
            -- Offline / startup check or unverified sync: ONLY delete if expiry date + grace period elapsed
        ...
        -- Critical safeguard: never auto-delete an actively open document
        if should_delete and tracked.path and isDocumentOpen(tracked.path) then
            logger.info("libbee autoclean: skipping auto-delete for " .. tostring(loan_id) ..
                        " ('" .. tostring(tracked.title) .. "') because document is actively open in ReaderUI")
            should_delete = false
        end
